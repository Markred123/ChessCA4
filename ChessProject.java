...
private Stack getWhitePawnSquares(int x, int y, String piece){
  ...
  Square startingSquare1 = new Square(x, y, piece);
//declaring the possiuble valid white pawn moves using the validwpX variables
  Move validwpMove1, validwpMove2, validwpMove3, validwpMove4;
  //declaring the temp x and y variables
  int tmpx1 = x+1;
  int tmpx2 = x-1;
  int tmpy1 = y+1;
  int tmpy2 = y+2;
  //These are all the possible moves for the white pawn declared as squares
  //this square has no X movement and just Y movement, moves the piece one square down
  Square tSquare = new Square(x, tmpy1, piece);
  // This square has the piece move 1 Y and 1 X, moving diagonally
  Square tSquare1 = new Square (tmpx1, tmpy1, piece);
  // this square moves the piece 2 on the Y axis and none on the X axis, move down 2
  Square tSquare2 = new Square (x, tmpy2, piece);
  //moves the piece x -1 and y 1, diagonally reversed
  Square tSquare3 = new Square (tmpx2, tmpy1, piece);

  //if the piece is in the starting position, which moves are possiuble
  // if there's no piece present in tSquare 3 AND there's no piece present in tSquare 1 then make validwpMove1 a move to tmpsquare 3
  if(y == 1){
    validwpMove1 = new Move(startingSquare1, tSquare2);
    // if there's no piece present in the square in front of the pawn or two squares in front of the pawn then the pawn can move two squares forward
    if(!piecePresent(((tSquare.getXC()*75)+ 20), (((tSquare.getYC()*75) + 20))) && !piecePresent(((tSquare2.getXC() * 75) + 20), (((tSquare2.getYC()* 75) + 20 )))){
      moves.push(validwpMove1);
    }
}

    //if the place it's moving to isn't off the board ( greater than 7 on the Y axis ) if there's no piece present in front of it, the move is valid, add to the stack
    if(!(tmpy1 > 7)){
      validwpMove2 = new Move(startingSquare1, tSquare);
      if(!piecePresent(((tSquare.getXC()* 75) + 20), (((tSquare.getYC()*75)+20)))){
        moves.push(validwpMove2);
      }
      // if the temp position isn't off the board on the X axis ( and Y axis as defined before) then moving diagonal right is valid, push to the stack
      if(!(tmpx1 > 7)){
      ...
      if(!(tmpx2 < 0)){
        validwpMove4 = new Move(startingSquare1, tSquare3);
        if(piecePresent(((tSquare3.getXC()*75)+20), (((tSquare3.getYC()*75)+ 20)))){
          if(checkWhiteOponent(((tSquare3.getXC()*75)+20), (((tSquare3.getYC()*75)+20)))){
            moves.push(validwpMove4);
          }
        }
      }
